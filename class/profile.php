...
class profile implements \jsonSerializable {
	...
	public static function getProfileByProfileId(\PDO $pdo, int $profileId) {
		...
		$query = "SELECT profileId,profileActivationToken,profileAthandle FROM profile WHERE profileId = :profileId";
	 ...
	 * @throws \PDOException when mySQL
	 */
	/**
	...
	public static function getProfileByProfileAtHandle(\PDO $pdo, string $profileAtHandle): \SplFixedArray {
		...
		$query = "SELECT profileId,profileActivationToken,ProfileAtHandle FROM profile WHERE profileAtHandle = :profileAtHandle";
		$statement = $pdo->prepare($query);
	...
	public static function getProfileByProfileActivationToken(\PDO $pdo, string $profileActivationToken): ?profile {
		...
		$query = "SELECT profileId,profileActivationToken,profileAtHandle FROM profile WHERE profileActivationToken = profileActivationToken";
		$statement = $pdo->prepare($query);
		...
		try {
			...
			$row = $statement->fetch();
			if($row !== false) {
				$profile = new profile($row["profileActivationToken"], $row{"profileAtHandle"});
			}
		} catch(\Exception $exception) {
			throw(new \PDOException($exception->getMessage(), 0, $exception));
		}
